...
def DoS_calc(path, abins=100, Rbins=30, maxTime=365.0):
    ...
    sim = MissionSim.MissionSim(scriptfile=path)
    ...
    amin = sim.PlanetPopulation.arange[0].to('AU').value
    amax = sim.PlanetPopulation.arange[1].to('AU').value
    Rmin = (sim.PlanetPopulation.Rprange[0]/const.R_earth).decompose().value
    assert Rmin < 45.0, 'Minimum planetary radius is above extrapolation range'
    if Rmin < 0.35:
        print 'Rmin reset to 0.35*R_earth'
        Rmin = 0.35
    Rmax = (sim.PlanetPopulation.Rprange[1]/const.R_earth).decompose().value
    ...
    WA = np.linspace(sim.OpticalSystem.IWA, sim.OpticalSystem.OWA, 50)
    mode = sim.OpticalSystem.observingModes[0]
    syst = mode['syst']
    lam = mode['lam']
    # contrast curve without astropy Quantities
    core_contrast = syst['core_contrast'](lam,WA)*sim.PostProcessing.ppFact(WA)
    ...
    opt = optimize.minimize_scalar(contrast, \
                                   bounds=[sim.OpticalSystem.IWA.to('arcsec').value,\
                                           sim.OpticalSystem.OWA.to('arcsec').value],\
                                           method='bounded')
    Cmin = opt.fun

    # find expected values of p and R
    if sim.PlanetPopulation.prange[0] != sim.PlanetPopulation.prange[1]:
        f = lambda p: p*sim.PlanetPopulation.dist_albedo(p)
        pexp, err = integrate.quad(f,sim.PlanetPopulation.prange[0],sim.PlanetPopulation.prange[1],epsabs=0,epsrel=1e-6,limit=100)
    else:
        pexp = sim.PlanetPopulation.prange[0]    
    if sim.PlanetPopulation.Rprange[0] != sim.PlanetPopulation.Rprange[1]:
        f = lambda R: R*sim.PlanetPopulation.dist_radius(R)
        Rexp, err = integrate.quad(f,sim.PlanetPopulation.Rprange[0].to('km').value,sim.PlanetPopulation.Rprange[1].to('km').value,epsabs=0,epsrel=1e-4,limit=100)
        Rexp *= sim.PlanetPopulation.Rprange.unit.to('AU')
    else:
        Rexp = sim.PlanetPopulation.Rprange[0].to('AU').value

    # include only F G K M stars
    spec = np.array(map(str, sim.TargetList.Spec))
    ...
    i = np.append(i,iM)
    i = np.unique(i)
    sim.TargetList.revise_lists(i)
    print 'Filtered target stars to only include M, K, G, and F type'

    # minimum and maximum separations
    smin = (np.tan(sim.OpticalSystem.IWA)*sim.TargetList.dist).to('AU').value
    smax = (np.tan(sim.OpticalSystem.OWA)*sim.TargetList.dist).to('AU').value
    smax[smax>amax] = amax
    
    # include only stars where smin > amin
    bigger = np.where(smin>amin)[0]
    sim.TargetList.revise_lists(bigger)
    smin = smin[bigger]
    smax = smax[bigger]
    
    # include only stars where smin < amax
    smaller = np.where(smin<amax)[0]
    sim.TargetList.revise_lists(smaller)
    smin = smin[smaller]
    smax = smax[smaller]
    
    print 'Beginning ck calculations'
    ck = find_ck(amin,amax,smin,smax,Cmin,pexp,Rexp)
    ...
    sInds = select_obs(sim.TargetList.tint0.to('day').value,maxTime,ck)
    print 'Finished ortools calculations'
    # include only stars chosen for observation
    sim.TargetList.revise_lists(sInds)
    smin = smin[sInds]
    smax = smax[sInds]
    ...
    result['Redges'] = Redges/const.R_earth.to('AU').value
    ...
    if len(Mlist) > 0:
        DoS['Mstars'] = DoS_sum(acents, aa, Rcents, RR, pexp, smin[Mlist], \
           smax[Mlist], sim.TargetList.dist[Mlist].to('pc').value, contrast)
    else:
    ...
    if len(Klist) > 0:
        DoS['Kstars'] = DoS_sum(acents, aa, Rcents, RR, pexp, smin[Klist], \
           smax[Klist], sim.TargetList.dist[Klist].to('pc').value, contrast)
    else:
    ...
    if len(Glist) > 0:
        DoS['Gstars'] = DoS_sum(acents, aa, Rcents, RR, pexp, smin[Glist], \
           smax[Glist], sim.TargetList.dist[Glist].to('pc').value, contrast)
    else:
    ...
    if len(Flist) > 0:
        DoS['Fstars'] = DoS_sum(acents, aa, Rcents, RR, pexp, smin[Flist], \
           smax[Flist], sim.TargetList.dist[Flist].to('pc').value, contrast)
    else:
    ...
    occ_rates['Mstars'] = find_occurrence(0.35*const.M_sun,ddP,ddR,Radii,\
             Periods,rates['MstarsMean'],aedges,Redges,\
                          sim.PlanetPopulation.dist_sma,amin)
    print 'Extrapolating occurrence rates for K stars'
    occ_rates['Kstars'] = find_occurrence(0.70*const.M_sun,ddP,ddR,Radii,\
             Periods,rates['KstarsMean'],aedges,Redges,\
                          sim.PlanetPopulation.dist_sma,amin)
    print 'Extrapolating occurrence rates for G stars'
    occ_rates['Gstars'] = find_occurrence(0.91*const.M_sun,ddP,ddR,Radii,\
             Periods,rates['GstarsMean'],aedges,Redges,\
                          sim.PlanetPopulation.dist_sma,amin)
    print 'Extrapolating occurrence rates for F stars'
    occ_rates['Fstars'] = find_occurrence(1.08*const.M_sun,ddP,ddR,Radii,\
             Periods,rates['FstarsMean'],aedges,Redges,\
                          sim.PlanetPopulation.dist_sma,amin)
    result['occ_rates'] = occ_rates
...
def DoS_sum(a,aa,R,RR,pexp,smin,smax,dist,Cs):
    ...
    for i in xrange(len(smin)):
        ...
        DoS += one_DoS_grid(aa,RR,pexp,smin[i],smax[i],Cmin)
        
    return DoS

def find_ck(amin,amax,smin,smax,Cmin,pexp,Rexp):
    '''Finds ck metric
...
def find_occurrence(Mass,ddP,ddR,R,P,Matrix,aedges,Redges,fa,amin):
